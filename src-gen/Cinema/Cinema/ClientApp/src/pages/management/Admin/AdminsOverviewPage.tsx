...
const AdminsOverviewPage = () => {
        ...
        const fetchData = async () => {
            ...
            setLoading(true);
            var result = await httpGet<Admin[]>("/Admin")
        ...
        const deleteEntity = async (id: String) => {
	                ...
	                var result = await httpDelete<boolean>(`/Admin?id=${id}`)
	                
	                if(result.isSuccess) {
                    	AdminResult.splice(AdminResult.indexOf(AdminResult.filter(e => e.id === id)[0]),1)
                                        ...
                                        <Table aria-label="simple table">
                                            ...
                                            {AdminResult.map((row) => (
                                                <TableRow key={row.id}>
                                                    ...
                                                    <TableCell>
                                                    	<Button color="primary" variant="outlined" onClick={() => history.push(`/management/Admin_update/${row.id}`)}>Edit</Button>
                                                    </TableCell>
                                            		<TableCell>
